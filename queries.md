...
8.  The star first name, star last name, and movie title for every
    ...
    SELECT first_name, last_name, title FROM stars INNER JOIN roles ON stars.id=roles.star_id INNER JOIN movies ON movies.id=roles.movie_id
...
9.  The first and last names of every star who has been in a G-rated movie. The first and last name should appear only once for each star, even if they are in several G-rated movies. *IMPORTANT NOTE*: it's possible that there can be two *different* actors with the same name, so make sure your solution accounts for that.\
    ...
    SELECT first_name, last_name
        ...
        WHERE movies.rating='G'
        GROUP BY stars.first_name, stars.last_name, movies.rating;

10. The first and last names of every star along with the number
    of movies they have been in, in descending order by the number of movies. (Similar to #9, make sure
    that two different actors with the same name are considered separately).

    SELECT first_name, last_name, COUNT(movies.title) AS count
        FROM stars 
        INNER JOIN roles ON stars.id=roles.star_id 
        INNER JOIN movies ON movies.id=roles.movie_id
        GROUP BY stars.first_name, stars.last_name
...
11. The title of every movie along with the number of stars in
    ...
    SELECT title, COUNT(stars.first_name) AS count
        FROM movies
        INNER JOIN roles ON movies.id=roles.movie_id
...
12. The first name, last name, and average runtime of the five
    ...
    SELECT stars.first_name, stars.last_name, AVG(runtime) as avg_runtime
        FROM movies
        INNER JOIN roles ON movies.id=roles.movie_id
        INNER JOIN stars ON roles.star_id=stars.id
        GROUP BY stars.first_name, stars.last_name
